...
@Composable
fun UpdatePage(navController : NavController,viewModel: TransactionViewModel, transactionId : Int) {
    ...
    val currentTransaction = viewModel.getTransactionById(transactionId)
    ...
    var isChecked1 by remember { if (currentTransaction?.type == Type.EXPENSE) {
        mutableStateOf(true) }
        else mutableStateOf(false)
    }
    var isChecked2 by remember {
        if (currentTransaction?.type == Type.INCOME) {
            mutableStateOf(true) }
        else mutableStateOf(false)
    }
