...
from PracticWork3.src.Exceptions.exception import *
from PracticWork3.src.PriorityQueue.priority_queue import PriorityQueue
...
class Graph:
    ...
    def minimum_cost_path(self, start_vertex, end_vertex):
        ...
        next = dict()  # a dictionary that has as the keys the vertices of the graph and as values the predecessor of the key in the lowest cost walks from
        ...
        if next[start_vertex] == -1:  # next we check if there is a walk between the start vertex and the end vertex, if not, we raise an exception
        ...
        while next[current_vertex] != -1:
            reverse_path.append(next[current_vertex])
