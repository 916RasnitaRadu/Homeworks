...
def sort_type(elements):
    """
  A function that creates a list of the types of expensive sorted ascending by amount of money.
  :param elements: a dictionary elements
  :return: a list of strings: list_type
  """
    list_type = []
    ...
    for amount in total_expenses:
        for type in elements:
            if type != "apartaments":
                params_list = [type]
                if amount == calculate_total_expenses_type(params_list, elements):
                    list_type.append(type)

    return list_type


def sort_apartament(elements):
    """
  A function that creates a list of the apartaments in the building sorted ascending by total amount of expenses.
  :param elements: a dictionary elements
  :return: a sorted list of integers: apartaments_sorted
  """

    apartaments_sorted = []

    total_expenses = calculate_total_expenses_for_all_apartaments(elements)
    # total_expenses_copy = total_expenses
    total_expenses.sort()

    for i in range(len(elements["apartaments"])):
        apartaments_sorted.append(0)

    for apartament_nr in elements["apartaments"]:
        amount = calculate_total_expenses_apartament(elements, apartament_nr)
        i = total_expenses.index(amount)
        apartaments_sorted[i] = apartament_nr
    """
  for j in range(len(elements["apartaments"])):
    amount = total_expenses_copy[j]
    i = total_expenses.index(amount)
    apartaments_sorted[i] = get_ap(elements, "apartaments", j)
  """

    """
  for amount in total_expenses:
    for apartament_nr in elements["apartaments"]:
      if amount == calculate_total_expenses_apartament(elements, apartament_nr):
        apartaments_sorted.append(apartament_nr)
  """

    return apartaments_sorted
...
def find_max_expense_heating(elements, history_list, apartament_nr):
    """
   A function that finds the maximum amount for the heating expense type in the entire history of transactions.
  :param elements: a dictionary, elements
  :param history_list: a list of dictionaries, representing the history of all transactions that were done.
  :param apartament_nr:an integer, representing the number of the given apartament
  :return: an integer, representing the maximum amount for the heating expense type in the entire history of
  transactions
  """
    heating_expenses = []
    for elem in history_list:
        i = get_apartament_index(elem, apartament_nr)
        heating_expenses.append(get_ap(elem, "heating", i))
    maxi = max(heating_expenses)
    return maxi


def find_max_expense_gas(elements, history_list, apartament_nr):
    """
  A function that finds the maximum amount for the gas expense type in the entire history of transactions.
  :param elements: a dictionary, elements
  :param history_list: a list of dictionaries, representing the history of all transactions that were done.
  :param apartament_nr: an integer, representing the number of the given apartament
  :return: an integer, representing the maximum amount for the gas expense type in the entire history of transactions
  """
    gas_expenses = []
    for elem in history_list:
        i = get_apartament_index(elem, apartament_nr)
        gas_expenses.append(get_ap(elem, "gas", i))
    maxi = max(gas_expenses)
    return maxi


def find_max_expense_electricity(elements, history_list, apartament_nr):
    """
   A function that finds the maximum amount for the electricity expense type in the entire history of transactions.
  :param elements: a dictionary, elements
  :param history_list: a list of dictionaries, representing the history of all transactions that were done.
  :param apartament_nr:an integer, representing the number of the given apartament
  :return: an integer, representing the maximum amount for the electricity expense type in the entire history of
  transactions
  """
    electricity_expenses = []
    for elem in history_list:
        i = get_apartament_index(elem, apartament_nr)
        electricity_expenses.append(get_ap(elem, "electricity", i))
    maxi = max(electricity_expenses)
    return maxi


def find_max_expense_other(elements, history_list, apartament_nr):
    """
  A function that finds the maximum amount for the other expense type in the entire history of transactions.
  :param elements: a dictionary, elements
  :param history_list: a list of dictionaries, representing the history of all transactions that were done.
  :param apartament_nr: an integer, representing the number of the given apartament
  :return: an integer, representing the maximum amount for the other expense type in the entire history of transactions
  """
    other_expenses = []
    for elem in history_list:
        i = get_apartament_index(elem, apartament_nr)
        other_expenses.append(get_ap(elem, "other", i))
    maxi = max(other_expenses)
    return maxi


def max_expense_for_each_type(elements, history_list, params_list):
    """
  A function that finds the maximum amount for each type of expense in the entire history of transactions and creates
  a list with the values.
  :param elements: a dictionary, elements
  :param history_list: a list of dictionaries, representing the history of all transactions that were done.
  :param params_list: a list of parameters, strings, params_list
  :return: a list of integers, representing the maximum amounts for each type of expense
  """
    apartament_nr = int(params_list[0])
    if apartament_nr in elements["apartaments"]:
        max_expense_water = find_max_expense_water(elements, history_list, apartament_nr)
        max_expense_heating = find_max_expense_heating(elements, history_list, apartament_nr)
        max_expense_gas = find_max_expense_gas(elements, history_list, apartament_nr)
        max_expense_electricity = find_max_expense_electricity(elements, history_list, apartament_nr)
        max_expense_other = find_max_expense_other(elements, history_list, apartament_nr)
        return [max_expense_water, max_expense_heating, max_expense_gas, max_expense_electricity, max_expense_other]
    else:
        return False
